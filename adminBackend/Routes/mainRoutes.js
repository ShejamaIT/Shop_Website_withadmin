...
router.get("/orders-Processing-stid", async (req, res) => {
    try {
        ...
        const query = `
            ...
            FROM Orders o
                     LEFT JOIN accept_orders ao ON o.OrID = ao.orID
            WHERE o.orStatus = 'Processing' AND stID = ?
        `;
        ...
        console.error("Error fetching Processing orders:", error.message);
        return res.status(500).json({ message: "Error fetching complProcessingeted orders", error: error.message });
    }
});
...
router.put("/update-invoice", async (req, res) => {
    try {
        ...
        const [orderResult] = await db.query(
            `SELECT OrID, orStatus, stID FROM Orders WHERE OrID = ?`,
            [orID]
        );
        ...
        const { orStatus, stID } = orderResult[0] || {};
        ...
        // 🛑 If pickup, delete from delivery
        if (isPickup) {
        ...
        if (addedAdvance > 0) {
            const netAdvance = totalAdvance - updatedDeliveryCharge;

            await db.query(
                `INSERT INTO order_payment (op_ID, orID, amount, dateTime, or_status, netTotal, stID)
                 VALUES (?, ?, ?, NOW(), ?, ?, ?)`,
                [op_ID, orID, addedAdvance, orStatus, netAdvance, stID]
            );
